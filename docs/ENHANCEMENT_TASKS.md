...
- **No real thumbnail filmstrip.** The reference screenshot's frame-thumbnail strip needs a backend ffmpeg extraction endpoint that doesn't exist yet — not faked with placeholder images; flagged as a follow-up instead.
- **Segments can only be retimed/resized/muted/deleted/retexted — not split, merged, or reordered.** Reordering wouldn't make sense for chronological speech; split/merge would be genuinely useful (e.g. one Whisper segment covering two sentences you want to time separately) but wasn't built this pass.
- **Zero live testing.** This is a large change — new migration, two new job classes, three new endpoints, a new frontend component with real drag/resize math — and none of it has run against a live browser or actual queue workers. Before relying on this: run the migration, restart `api`/rebuild frontend, submit a real dub, and specifically verify (1) the review timeline actually loads and renders segments at the right position/width for the zoom level, (2) drag-to-retime and edge-resize behave correctly and respect the neighbor-clamping, (3) Save persists correctly (reload the page and confirm edits stuck), (4) Generate actually kicks off `FinalizeDubbingJob` and the final video reflects the edited timings/text, and (5) a deliberately-broken finalize (e.g. wrong engine) reverts to `ready_for_review` instead of `failed` and doesn't burn a second translation credit on retry.
